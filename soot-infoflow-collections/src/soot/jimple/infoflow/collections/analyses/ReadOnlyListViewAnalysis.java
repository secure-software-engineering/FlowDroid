...
/**
 * Intraprocedural analysis that reasons whether a list view (i.e. iterator or sublist) modifies
 * the list.
 *
 * @author Tim Lange
 */
    ...
    private boolean isReadOnlyIteratorInternal(SimpleLocalUses du, AssignStmt assign) {
        for (UnitValueBoxPair uv : du.getUsesOf(assign)) {
            ...
            if (stmt instanceof AssignStmt) {
                ...
                // Fields are not supported, so we will bail out if a list or iterator is assigned
                // to a field. For other types, we don't care because they can't change the list.
                if (!isLocal)
                    if (isSupportedClass)
                        return false;
                    else
