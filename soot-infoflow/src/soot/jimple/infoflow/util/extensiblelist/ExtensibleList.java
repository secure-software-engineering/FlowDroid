...
public class ExtensibleList<T> {
	...
	/**
	 * Creates a new extensible list. The given parameter is the prepended to
	 * this list.
	 * 
	 * @param original
	 *            the prepended list
	...
	/**
	 * Returns the aggregated hash code of each element, including the elements
	...
	/**
	 * In case this list is locked, a new one is created and returned. So,
	 * always use the returned list!
	 * 
	 * @param add
	 *            the object to add
	 * @return the new list
	...
	public ExtensibleList<T> add(T add) {
		...
		// printStats();
		/*
		 * if (check != null) { check.add(add); if (savedHashCode !=
		 * Integer.MIN_VALUE) savedHashCode ^= add.hashCode(); check(); }
		 */
		return this;
	}
	 ...
	 * 
	 * ExtensibleListIterator<T> it = reverseIterator(); for (int x =
	 * check.size() - 1; x >= 0; x--) { if (!it.hasNext()) throw new
	 * AssertionError(); if (!it.next().equals(check.get(x))) throw new
	 * AssertionError(); } if (it.hasNext()) throw new AssertionError(); }
	 */
	 ...
	 * 
	 * @return the last element (or null) or an extensible list in case it
	...
	private Object getOrRemoveLast(boolean remove) {
		...
		try {
				...
				if (check.actualList != null && !check.actualList.isEmpty()) {
					...
					if (remove) {
						if (elem < check.lastLocked) {
							// Too bad... We cannot remove the element, since it
							// is locked (some other list may need it).
			...
			if (updateHash) {
				// we have changed one list, so let's update all hashcodes from
				// here up to
				// this list
				// since this list was not locked, we do not have to worry about
				// other lists,
				// since no other list should dependent on that list where we
				// removed the element.
	...
	/**
	 * Returns the first element. It is relatively slow compared to getting the
	 * last element. This method should not be used in normal path
	 * reconstruction.
	 * 
	...
	/**
	 * Adds an element to the beginning. Since this data structure was not meant
	 * to prepend an element, it is relatively slow, so be careful. Does not
	 * change this list, but returns a new list! This method should not be used
	 * in normal path reconstruction.
	 * 
	 * @param toAdd
	 *            the element to add
	 * @return the new list
