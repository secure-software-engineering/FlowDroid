...
/**
 * Rule that skips over certain often-used system classes which do not modify the
 * taint state
 * 
 * @author Steven Arzt
 *
...
public class SkipSystemClassRule extends AbstractTaintPropagationRule {
	...
	public SkipSystemClassRule(InfoflowManager manager, Aliasing aliasing,
	...
	/**
	 * Gets whether the given destination method is one of our well-known
	 * system methods
	...
	private boolean isSystemClassDest(SootMethod dest) {
		return dest == objectCons || dest == objectClinit || dest == objectGetClass
				|| dest == threadCons;
	}
	...
	public Collection<Abstraction> propagateCallToReturnFlow(Abstraction d1,
			...
			ByReferenceBoolean killAll) {
		// If we don't have any callees, we may not interfere with the normal propagation
