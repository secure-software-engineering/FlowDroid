...
public class TaintPropagationResults {
	...
	protected final MyConcurrentHashMap<AbstractionAtSink, TaintAbstraction> results = new MyConcurrentHashMap<AbstractionAtSink, TaintAbstraction>();
	...
	public boolean addResult(AbstractionAtSink resultAbs) {
		...
		TaintAbstraction abs = resultAbs.getAbstraction();
		abs = abs.deriveNewAbstraction(abs.getAccessPath(), resultAbs.getSinkStmt());
		abs.deriveSourceContext(resultAbs.getAbstraction());

		// Reduce the incoming abstraction
		IMemoryManager<AbstractDataFlowAbstraction, Unit> memoryManager = manager.getForwardSolver().getMemoryManager();
		if (memoryManager != null) {
			abs = (TaintAbstraction) memoryManager.handleMemoryObject(abs);
			if (abs == null)
				return true;
		}
		...
		this.results.putIfAbsent(resultAbs, resultAbs.getAbstraction());
	...
	/**
	 * Clears all results registered in this data object
	 */
	public void clear() {
		this.results.clear();
	}
