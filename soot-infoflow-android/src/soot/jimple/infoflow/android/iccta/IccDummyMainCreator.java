...
public class IccDummyMainCreator {
	...
	/**
	 * since FlowDroid already resolved this problem, they will generate a
	 * dummyMain method for all comp class. but here, we want to generate
	 * dummyMain method for each comp class. this is kind of special use.
	 * 
	 * @param sootClassName
	 * @return
	 */
	...
	public SootMethod generateDummyMainMethod(List<String> entryPoints, String sootClassName) {
		final SootClass compSootClass = Scene.v().getSootClass(sootClassName);
		...
		AndroidEntryPointCreator androidEPCreator = new AndroidEntryPointCreator(androidClasses) {
			...
			protected void createEmptyMainMethod() {
				// Create a new dummy main method for the component at hand
				mainMethod = new SootMethod(DUMMY_MAIN_METHOD, new ArrayList<Type>(), VoidType.v(), Modifier.PUBLIC);
				JimpleBody body = Jimple.v().newBody(mainMethod);
				mainMethod.setActiveBody(body);
				compSootClass.addMethod(mainMethod);

				// this is mandatory, the default dummyMainMethod is static, so
				// they do not deal thisIdentity. since we don't need static
				// dummyMainMethod, we should define it explicit
				body.insertIdentityStmts();
			}

		};
		androidEPCreator.setCallbackFunctions(callbackFunctions);
		return androidEPCreator.createDummyMain();
	}
